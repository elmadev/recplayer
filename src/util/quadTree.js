...
export default function quadTree(minW) {
  ...
  function add(valx, valy, val) {
    ...
    if (add_({ x: valx, y: valy, val: val }, root, 0, 0, rootW) !== root)
      throw new Error(
        "internal error: a gyökér csomópontoknak egyezniük kell!"
      ); // hehe
  }
  ...
  function add_(desc, tree, x, y, w) {
    return tree({
      ...
      tip: function(descs) {
        ...
        for (var z = 0; z < descs.length; z++) r = add_(descs[z], r, x, y, w);
  ...
  function traverse(x, y, w, h, fn) {
    traverse_(root, 0, 0, rootW, x, y, w, h, function(desc) {
      if (desc.x >= x && desc.y >= y && desc.x < x + w && desc.y < y + h)
